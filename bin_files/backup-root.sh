#!/bin/bash
# This script will attempt to mount a backup media and backup root
# After the backup, the system will shut down

# Add /sbin to path to allow shutdown command
PATH=$PATH:/sbin

# Check if device is plugged in:
if [ -e /dev/disk/by-label/BAKROOT ]; then
  # MOUNT BACKUP DRIVE AND PREPARE DRIVES
  DATE=$(date +"%Y-%m-%d %T")
  echo "$DATE : Mounting backup drive:"
  sync
  sleep 2
  mount /dev/disk/by-label/BAKROOT
  
  # BACKUP FILES AND FOLDERS
  # NOTE FOR MAKING RECOVERY SCRIPT, DON'T USE the option:
  # "--delete-excluded" option for recovery (unless
  # followed by recovery scripts which completely restore
  # the excluded directories!)
  #rsync -ahvv --delete --delete-excluded --delete-after \
  rsync -ahvv --delete-during --delete-excluded         \
    --exclude '/var' --exclude '/tmp' --exclude '/home' \
    --exclude '/proc' --exclude '/mnt'                  \
    / /mnt/BACKUP/ROOT/
  
  DATE=$(date +"%Y-%m-%d %T")
  echo "$DATE : Backup of / completed, unmounting external drive:"
  # WRITE ALL CHANGES TO DISK AND UNMOUNT BACKUP DRIVE
  sync
  sleep 2
  umount /mnt/BACKUP/ROOT

  DATE=$(date +"%Y-%m-%d %T")
  echo "$DATE : Root system backup routine completed"
else
  DATE=$(date +"%Y-%m-%d %T")
  echo "$DATE : Backup drive not found, aborting"
fi

# Since this script will be run Friday night as a cron job
# we want the system to shutdown after completion. Unless
# there is an active session.

ps -A|grep awesome > /dev/null
out=$?
if [[ out -eq 1 ]];then
  # assume no user logged in
  shutdown -h now
fi
